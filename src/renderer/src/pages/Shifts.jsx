...
import {
  Avatar, Form, notification, Spin, Tabs
} from "antd";
...
import {
  ...
  useGetShiftQuery, useGetShiftsQuery
} from "../api/shiftApiSlice";
import {
  CloseShift,
  CostRevenue,
  ListOrders,
  ListShifts, ShiftDetail,
  StartShift
} from "../components";
...
const Shifts = () => {
  ...
  const {
    ...
    data?.content[0]?.status ? data?.content[0]?.id : skipToken
  );

  const [closeShift, { isLoading: isLoadingCloseShift }] =
    useCloseShiftMutation();
              ...
              {!data?.content?.[0]?.status ? (
                ...
                <div className="w-[400px] rounded-lg border border-gray-200 p-4 text-sm hover:shadow-md">
                  <div className="-mx-4 flex items-center justify-between gap-1 border-b border-gray-200 px-4 pb-2">
                    ...
                    <p className="rounded-md bg-gray-300 py-1 px-2">
                      {dataShift?.code}
                    </p>
                  </div>
                  <div className="mt-2 flex items-center justify-between gap-1">
                    <div>Giờ mở ca:</div>
                    <p>
                      {dayjs(dataShift?.timeStartShifts).format(
                        "HH:mm DD/MM/YYYY"
                      )}
                    </p>
                  </div>
          ...
          <Tabs
            ...
            items={[
              ...
              },
              {
                ...
                key: "2",
                children: (
                  <CostRevenue shift={dataShift} isLoading={isLoading} />
                ),
              },
              {
                label: "Danh sách ca",
